...
struct proc {
  ...
  int priority;                // Process priority (-19-20); lower value, higher priority
  int tempPriority;
  mutex m[MAXPROCNAMELEN];   //pointer to array of lock holds
  int wm;                  //pointer to single wait for lock hold
  uint totalM;
};
typedef struct {
